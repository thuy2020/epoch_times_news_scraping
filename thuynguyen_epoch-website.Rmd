...
```{r testing}
# this link doesn't not give result because author name was not extracted --> need to use cbind instead of tible to gather all features for final result
...
```{r get_page_link function, all_article_links}
...
test_total <- unlist(list(test_correct_links, test_fixed_wrong_links)) # make them a list, then unlist to get correct length --> must be some better way to do this? 
```
...
total_links <- unlist(list(correct_links, wrong_links_fixed))

######## test
length(all_article_links) # 18657
length(wrong_links) # 1607
...
## Extracting on a test set of links
...
View(results)
dim(results) # should have 5 rows, not just 1
```
