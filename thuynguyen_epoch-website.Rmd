...
links_in_one_page <- get_article_links(url = "https://www.theepochtimes.com/c-us-politics/3")
```
...
for (url in links_in_one_page) {
  # Define a delayed read_html() function with a delay of 1s
  # read_html_delayed <- slowly(read_html, 
  #                           rate = rate_delay(1))
  # url <- read_html_delayed(url) --> not running???
  article <- get_article(url = url, save = TRUE, path = "test.html")
  result  <- get_features(article)
  
  # need to bind all ???
  print(result)
}
#problem:  some returns 0 row
```
...
# Get some page links, map through a vector of all pages 2:683
all_page_links <- map_chr(2:10, get_page_link) 

# get all article links from above page links, 
all_article_links <- map(all_page_links[4:5], get_article_links) %>% 
                    unlist() # to solve problem: list of list every 27 links nested in a list 

# --> problem: some links are not clean

for (url in all_article_links) {
  article <- get_article(url, save = TRUE, path = "test.html")
  result <- get_features(article)
  print(result)
}
 # --> problem: empty rows
