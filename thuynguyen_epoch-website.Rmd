...
## Progress made:- wrote functions to get articles links, page links, extracting article features of interest.
- Fixed empty rows problem
- Got all article links from 691 pages in the politics section. Stored this into an RDS file (all_article_links.RDS) for later use.
- Started to get the article features of interest using a set of articles links

## To do tasks: 
- Some article links are not clean, which prevents getting the exact article features. These links need to be sorted out and fixed.
- Extract all articles features of interest from the links in all_article_links.RDS.

## Would like to discuss:
- I'm trying to get the above 2 tasks by this weekend. I hope you will have time to give me some feedback then. 
- Should I adjust goal #2? I was thinking that after achieving goal #1, whether it is more appropriate to produce an interactive report rather than pulling up (rather simple) functions into a package.
...
#        publish_update = lubridate::mdy(gsub("Updated: ", "", publish_update)),
#        description = description,
#        content = post_content)
# return (tbl)
# ---> the above will not give result if an article has some missing features 

all_features <- cbind(title,
       autho_name,
       publish_date,
       publish_update,
       description,
...
# if the function uses tibble (or data_frame) to store output, apply such function to this article won't give any result because there's no author name --> need to use cbind to get all features -->then the problem is we don't have a nice tibble to work with --> need to convert to tibble
article1 <- get_article(url = "https://www.theepochtimes.com/democrats-introduce-bill-to-massively-expand-mail-in-voting_3676994.html") 
...
test_total <- unlist(list(test_correct_links, test_fixed_wrong_links)) 
```
...
total_links <- unlist(list(correct_links, wrong_links_fixed)) # make them a list, then unlist to get correct length --> must be some better way to do this? 

######## test
length(all_article_links) # 18657
length(wrong_links) # 1607 * 2
length(correct_links) # 17050
length(wrong_links_fixed) # 1607
length(total_links) # 18657
...
for (url in 1:length(test5)) {
  
 # delay function get_article to run slowly 0.5s
 get_article_delayed <- slowly(get_article, 
                             rate = rate_delay(0.5))
  
  articles <- get_article_delayed(url = test5[[url]])
  ...
  for (i in articles) {
    ...
    test5_list[[i]] <- art_features

  }
  
}

test5_unlist <- unlist(test5_list)
typeof(test5_list)
length(test5_unlist) # 5 articles, 6 features, why length of 25? --> some features are NA. Eg. the 1st articles has 4, the second 5, the third 6, 

###############3
# Tried this but failed
# library(magicfor)
# magic_for(get_features_delayed)
# magic_for(get_article_delayed)
# 
# for (url in 1:length(test5)) {
#   
#  # delay function get_article to run slowly 0.5s
...
#                              rate = rate_delay(0.5))
#   
#   articles <- get_article_delayed(url = test5[[url]], save = TRUE, path = "test.html")
#     
#   for (i in articles) {
#     # delay function get_features to run slowly 1s
#     get_features_delayed <- slowly(get_features,
...
#   
# }
# magic_result_as_dataframe() --> SUPER slow, get a column of url and 5 rows 1, 2, 3, 4, 5
```
...
```{r sentiment analysis on test5_unlist}
library(tm)
library(qdap) # just can't get this. 


test_source <- VectorSource(test5_unlist)
# create a volatile corpus
test_corpus <- VCorpus(test_source)

# the 10th document object, [1] to extract the content, [2] for metatdata 
test_corpus[[10]][1]
# review the text of the article
text <- content(test_corpus[[4]]) # try and error to find out the position of article body text, must be a better way

my_stopwords <- c("president", stopwords("en"))
removeWords(text, my_stopwords)
...
typeof(test5_list) # typeof list, but weird
str(test5_unlist)
```
